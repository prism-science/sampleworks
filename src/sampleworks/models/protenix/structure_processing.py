                ...
                if chain_atom[0].label_entity_id == label_entity_id:
                    ...
                    if chain_type.is_polymer():
                        ...
                        if valid_positions is not None and chain_id in valid_positions:
                            chain_valid = valid_positions[chain_id]
                            if chain_has_seq_idx:
                                # Insertion-coded residues can share a raw res_id,
                                # retain every corresponding full-sequence position.
                                raw_res_ids = cast(np.ndarray, chain_array.res_id)[starts[:-1]]
                                valid_seq_positions = {
                                    pos
                                    for res_id, pos in zip(raw_res_ids, positions, strict=True)
                                    if res_id in chain_valid
                                }
                            else:
