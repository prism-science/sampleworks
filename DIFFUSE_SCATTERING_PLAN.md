...
lunus is **not declared in `pyproject.toml`**. Install it with
`scripts/install_lunus.sh`, which pins the same rev.

That is a workaround for a workspace-wide constraint, not a preference:

- **This workspace cannot be re-locked on any single machine.** Every
  environment carries `sampleworks` as an editable path dependency, and a source
  dependency must be *built* to yield metadata — per platform. macOS fails
  solving linux-64; the Linux runner fails solving osx-arm64 for `boltz-osx`.
  `pixi lock` is all-or-nothing, with no `--environment` or `--platform`
  selector, so neither can produce a complete lock. Declaring any new dependency
  therefore requires a lock nobody can generate. This predates lunus and is not
  specific to it.
- **It worked once.** The relock that first added lunus succeeded and resolved
  it into all 13 environments including osx-arm64 ones. Every attempt since has
  failed at `boltz-osx`. The likeliest explanation is that adding a dependency
  is an incremental update while moving one between features forces a full
  re-resolve — unverified.
- **Two ways out, both outside this work.** Drop `osx-arm64` from the workspace
  (a repo-owner decision — CI never builds macOS, so the platform exists purely
  for local development), or have lunus publish a wheel: `lunus.sf` is pure
  Python, so one `py3-none-any` artifact needs no build anywhere and the
  constraint disappears for this dependency at least.
